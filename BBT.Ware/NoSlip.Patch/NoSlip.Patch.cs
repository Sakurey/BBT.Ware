...
namespace BBT.Ware.AntiSLip;

public sealed class BbtAntiSlipSystem : EntitySystem
{
    ...
    {
        if (BBTConfigMovement.AntiSlip && !enabled && _forcePressWalk) 
            PressWalk(BoundKeyState.Down);
        BBTConfigMovement.AntiSlip = enabled;
    }
    ...
    {
        if (!_playerManager.LocalEntity.HasValue || !BBTConfigMovement.AntiSlip)
            return;
